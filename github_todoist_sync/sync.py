...
def main():
    ...
    update_tasks(
        ...
        item_task_mapping=item_task_mapping,
        repo_to_project_id=repo_to_project_id,
    )


def save_mapping(mappings_file: Path, item_task_mapping: dict):
    with open(mappings_file, "w") as f:
        json.dump(item_task_mapping, f, indent=4)


def get_issues(github: Github, username: str) -> list:
    issues = []
    # Issues assigned to me
    assigned_issues = github.search_issues(
        f"assignee:{username} is:open is:issue", sort="updated", order="desc"
    )
    # Issues created by me
    created_issues = github.search_issues(
        f"author:{username} is:open is:issue", sort="updated", order="desc"
    )
    # Issues where I am mentioned
    mentioned_issues = github.search_issues(
...
def create_todoist_task(
    ...
    relation: str,
    item: Issue,
    repo_to_project_id: dict,
    mapping_file: Path,
    item_task_mapping: dict,
    todoist: TodoistAPI,
):
    ...
    try:
        ...
        save_mapping(mappings_file=mapping_file, item_task_mapping=item_task_mapping)
...
def update_tasks(
    ...
    issues = get_issues(github, username)
    prs = get_pull_requests(github, username)
        ...
        for item_id in closed_item_ids:
            ...
            try:
                ...
                if not task.is_completed:
                    ...
                    print(f"Task {task_id} is already completed, Removing from mapping")
                    del item_task_mapping[item_id]
                    save_mapping(
                        mappings_file=mappings_file, item_task_mapping=item_task_mapping
                    )
            except Exception as e:
